...
const Home = ({ data }) => {
  ...
  return (
    <div className="flex min-h-screen flex-col items-center justify-center bg-yellow-300">
      ...
      <Navbar />
      <main className="flex w-full flex-1 flex-col items-center content-start px-16 text-center">
        ...
        <div className="mt-4 flex w-full items-center justify-around sm:w-full">
          <div className='bg-cyan-400 w-3/4 h-auto px-2 py-2'>
            <h1 className='text-3xl font-bold text-left'>Last manga updates</h1>
            <div className='grid grid-cols-5 auto-cols-max gap-2'>
              {updatedManga}
            </div>
          </div>
          <div className='bg-cyan-400 w-1/4'>
            <h1 className='text-3xl font-bold'>Top rated manga</h1>
...
export const getServerSideProps = async () => {
  // const resp = await fetch("http://localhost:8080/api/manga/most-viewed");
  const URL = "http://localhost:8080/api/manga/last-updated";
